...
const CATS = [
  ...
  { key: 'Health and Beauty', label: 'Health & Beauty', icon: '💄' },
  { key: 'Phones and Tablets', label: 'Phones & Tablets', icon: '📱' },
  { key: 'Electronics', label: 'Electronics', icon: '🔌' },
  { key: 'Fashion', label: 'Fashion', icon: '👗' },
  { key: 'Toys and Games', label: 'Toys & Games', icon: '🎮' },
  { key: 'Sporting Goods', label: 'Sporting Goods', icon: '⚽' },
  ...
  { key: 'Garden and Outdoors', label: 'Garden & Outdoors', icon: '🌿' },
...
export default function Shop() {
  ...
  return (
    <div style={{ fontFamily: 'Georgia, serif', color: '#2C2C2A', minHeight: '100vh', background: '#F5F5F5' }}>
      ...
      {/* Category scroll */}
      <div style={{ background: '#fff', borderBottom: '1px solid #E0E0E0', overflowX: 'auto', display: 'flex', padding: '10px 16px', gap: 8, scrollbarWidth: 'none' }}>
        {CATS.map(cat => (
          <button key={cat.key} onClick={() => handleCat(cat.key)}
            style={{ flexShrink: 0, display: 'flex', alignItems: 'center', gap: 5, padding: '6px 14px', borderRadius: 20, border: `1.5px solid ${activeCat === cat.key ? '#2C5F2E' : '#E0E0E0'}`, background: activeCat === cat.key ? '#EAF3DE' : '#fff', cursor: 'pointer', fontSize: 12, fontWeight: activeCat === cat.key ? 700 : 400, color: activeCat === cat.key ? '#1A3D1C' : '#5F5E5A', whiteSpace: 'nowrap', fontFamily: 'Georgia, serif' }}>
      ...
      {/* Toolbar */}
      <div style={{ background: '#fff', padding: '10px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #E0E0E0' }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <button onClick={() => setFiltersOpen(!filtersOpen)}
            style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '5px 12px', border: `1.5px solid ${filtersOpen ? '#2C5F2E' : '#E0E0E0'}`, borderRadius: 16, background: filtersOpen ? '#EAF3DE' : '#fff', cursor: 'pointer', fontSize: 12, fontFamily: 'Georgia, serif', color: filtersOpen ? '#2C5F2E' : '#5F5E5A', fontWeight: filtersOpen ? 700 : 400 }}>
            ⚙️ Filter
          </button>
          <select value={sort} onChange={e => setSort(e.target.value)}
            style={{ fontSize: 12, border: '1px solid #E0E0E0', borderRadius: 16, padding: '5px 10px', outline: 'none', fontFamily: 'Georgia, serif', color: '#5F5E5A', background: '#fff', cursor: 'pointer' }}>
      ...
      {/* Filter panel */}
      {filtersOpen && (
        <div style={{ background: '#fff', padding: '14px 16px', borderBottom: '1px solid #E0E0E0' }}>
          ...
          <button onClick={() => { setMaxPrice(50000); setSearch(''); setActiveCat('all'); navigate('/shop'); }}
      ...
      <div style={{ padding: 10 }}>
        {loading ? (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8 }}>
            {[...Array(6)].map((_, i) => (
              <div key={i} style={{ background: '#fff', borderRadius: 8, overflow: 'hidden', border: '1px solid #E0E0E0' }}>
                <div style={{ height: 130, background: 'linear-gradient(90deg,#f0f0f0 25%,#e0e0e0 50%,#f0f0f0 75%)' }} />
                <div style={{ padding: 8 }}>
                  <div style={{ height: 12, background: '#f0f0f0', borderRadius: 4, marginBottom: 6, width: '80%' }} />
                  <div style={{ height: 14, background: '#f0f0f0', borderRadius: 4, width: '50%' }} />
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{ textAlign: 'center', padding: '60px 20px' }}>
            ...
            <button onClick={() => { setSearch(''); setActiveCat('all'); setMaxPrice(50000); navigate('/shop'); }}
            ...
            {filtered.map(p => {
              ...
              return (
                <div key={p._id}
                  ...
                  style={{ background: '#fff', border: '1px solid #E8E8E8', borderRadius: 8, overflow: 'hidden', cursor: 'pointer', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
                  <div style={{ height: 130, background: '#F8F8F8', position: 'relative', overflow: 'hidden' }}>
                    {disc && (
                      <div style={{ position: 'absolute', top: 4, left: 4, background: '#BA7517', color: '#fff', borderRadius: 4, padding: '1px 5px', fontSize: 9, fontWeight: 700, zIndex: 1 }}>
                        -{disc}%
                      </div>
                    )}
                    {!p.inStock && (
                      <div style={{ position: 'absolute', inset: 0, background: 'rgba(255,255,255,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2 }}>
                        <span style={{ background: '#888', color: '#fff', borderRadius: 6, padding: '3px 8px', fontSize: 10, fontWeight: 700 }}>Sold Out</span>
                      </div>
                    )}
                    {p.images?.[0]
                  ...
                  <div style={{ padding: '6px 8px 8px' }}>
                    <div style={{ fontSize: 10, color: '#999', textTransform: 'uppercase', letterSpacing: '0.3px', marginBottom: 2 }}>{p.category}</div>
                    <div style={{ fontSize: 11, fontWeight: 600, lineHeight: 1.3, marginBottom: 5, color: '#222', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                      {p.name}
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 4 }}>
                      <div>
                        ...
                        {p.oldPrice && p.oldPrice > p.price && (
                          <div style={{ fontSize: 9, color: '#999', textDecoration: 'line-through' }}>{fmt(p.oldPrice)}</div>
                        )}
                      </div>
                      <button
                        onClick={e => handleAdd(e, p)}
                        disabled={!p.inStock}
                        style={{ background: addedId === p._id ? '#BA7517' : p.inStock ? '#2C5F2E' : '#ccc', color: '#fff', border: 'none', borderRadius: 6, padding: '5px 8px', fontSize: 11, fontWeight: 700, cursor: p.inStock ? 'pointer' : 'not-allowed', whiteSpace: 'nowrap', flexShrink: 0 }}>
                        {addedId === p._id ? '✓' : '+ Cart'}
                      </button>
                    </div>
                  </div>
      ...
      {/* WhatsApp CTA */}
      <div style={{ margin: '10px 10px 24px', background: '#EAF3DE', border: '1px solid #C0DD97', borderRadius: 12, padding: '14px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
        <div>
        ...
        <a href={`https://wa.me/${process.env.REACT_APP_SHOP_WHATSAPP || '254746251745'}?text=Hi! I'm looking for a product not on your website.`}
